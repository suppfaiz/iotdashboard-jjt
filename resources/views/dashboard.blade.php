            ...
            @foreach($activeWarnings as $warning)
                <div class="flex items-center justify-between p-3 rounded-2xl bg-white/70 border border-rose-100 text-slate-800 text-xs font-semibold shadow-sm gap-4" id="alert-item-{{ $warning['device_id'] }}-{{ $warning['type'] }}">
                    ...
                    @if(!empty($electricianWhatsapp))
                        <a href="https://wa.me/{{ $electricianWhatsapp }}?text=Halo%20Tukang%20Listrik%2C%20terdeteksi%20masalah%20pada%20perangkat%20{{ urlencode($warning['device_name']) }}%3A%20{{ urlencode($warning['message']) }}" target="_blank" class="flex-shrink-0 px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border border-emerald-100 rounded-lg transition-colors flex items-center gap-1 font-bold text-[10px]">
